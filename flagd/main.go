...
func main() {
	...
	wasmWrapper, err := host.NewIoStreamClient(ctx, flagdWasm, flagsInitial)
	if err != nil {
		fmt.Println(err)
	}

	// set a new flag configuration against the running wasm module
	res, err := wasmWrapper.Call(ctx, "FLAG_CONFIGURATION", flagsNew)
	if err != nil {
		fmt.Println(err)
	}
	// print the response
